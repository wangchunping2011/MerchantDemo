...
#import "DCPChineseSort.h"
//特殊字符统一合并后的  显示字符
#define AbnormalLetter @"#"

@interface DCPChineseSort()
...
+(NSMutableArray*)IndexArray:(NSArray*)stringArr{
    ...
    {
        NSString *pinyin = [((DCPChineseSort*)object).pinYin substringToIndex:1];
        //不同
...
+(NSMutableArray*)LetterSortArray:(NSArray*)stringArr{
    ...
    for (NSString* object in tempArray) {
        ...
        NSString *pinyin = [((DCPChineseSort*)object).pinYin substringToIndex:1];
        NSString *string = ((DCPChineseSort*)object).string;
        //不同
        if(![tempString isEqualToString:pinyin])
        {
            //分组
...
+(NSMutableArray*)ReturnSortChineseArrar:(NSArray*)stringArr{
    ...
    {
        DCPChineseSort *chineseString = [[DCPChineseSort alloc]init];
        ...
        chineseString.string = [DCPChineseSort RemoveSpecialCharacter:chineseString.string];
...
+(NSMutableArray*)sortObjectArray:(NSArray*)objectArray Key:(NSString *)key{
    ...
    NSString *tempString;
    //拼音分组
    for (DCPChineseSort* object in tempArray) {
